classdef AwgnEstimOut < EstimOut
    ...
    methods
        ...
        function [zhat, zvar, partition] = estim(obj, phat, pvar)
            ...
            wvar = obj.wvar;
            %length(wvar(isnan(wvar)))
            %length(pvar(isnan(pvar)))
            %length(scale2pvar(isnan(scale2pvar)))
            gain = pvar./(scale2pvar + wvar);
            %max(phat)
            %min(phat)
            %max(pvar)
            %min(pvar)
            %length(gain(isnan(gain)))
            %sum(abs(pvar))

            zhat = (scale*gain).*(y-scale*phat_real) + phat_real;
            %if (any(isnan(zhat)))
            %    idx_nan=1:size(zhat,1);
            %    idx_nan = idx_nan(isnan(zhat));
            %    fprintf('%5d   %5d   %5d\n', pvar(idx_nan(1)), scale2pvar(idx_nan(1)), wvar(idx_nan(1)));
            %end
            zvar = wvar.*gain;
