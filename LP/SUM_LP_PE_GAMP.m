...
% Sum-product message passing with Laplace prior input channel
%
...
function [Xhat, PEfin, estHist] = SUM_LP_PE_GAMP(Y, A, optPE, optGAMP)
...
while stop == 0
    ...
    if ~optPE.cmplx_in
        inputEst = SoftThreshEstimIn(lambda, false);
    else
        % TBD
    end
    ...
    if ~optPE.cmplx_in
        %Update parameters
        [lambda, lambda_all] = sum_lp_input_update(real(Rhat), real(Rvar), lambda, lambda_all);
    else
        % TBD
    end
